...
WORKDIR /src
COPY ["UssdApp.fsproj", "UssdApp/"]
RUN dotnet restore "UssdApp/UssdApp.fsproj"

WORKDIR "/src/UssdApp"
COPY . .
RUN dotnet build "UssdApp.fsproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "UssdApp.fsproj" \
...
Execute unit tests
```
cd "UssdApp/test/UssdApp.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "UssdApp/src/UssdApp"
